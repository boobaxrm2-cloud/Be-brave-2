...
let Users, Students, Teachers, Lessons, Files, Notes, Certificates;
...
app.put('/api/lessons/:id', auth, isTeach, (req, res) => {
  ...
  ['status','meetLink','topic','subject','date','time','duration'].forEach(k => { if (req.body[k] !== undefined) l[k] = req.body[k]; });
  Lessons.update(l);
  res.json(l);
});
