...
async function loadAdminStudents() {
  ...
  if (!students.length) { el.innerHTML = '<p class="empty">Nenhum aluno cadastrado ainda.</p>'; return; }
  el.innerHTML = `<table class="list-table"><thead><tr><th>Aluno</th><th>Matrícula</th><th>Nível</th><th>Professor</th><th>Aulas</th><th>Cadastrado em</th><th>Ações</th></tr></thead><tbody>
    ${students.map(s=>`<tr>
...
async function loadStudent() {
  ...
  document.getElementById('s-teacher-name').textContent = ME.teacherName || '—';
...
function renderStudentDashboard(lessons, notes) {
  ...
  renderStats('s-stats', [
    ...
    { icon:'🎯', val:subjects.length, lbl:'Assuntos estudados', cls:'bc-amber' },
  ...
  document.getElementById('s-history').innerHTML = done.length
    ? `<thead><tr><th>Data</th><th>Assunto</th><th>Detalhes</th><th>Duração</th><th>Meet</th></tr></thead><tbody>
        ${done.slice().reverse().map(l=>`<tr>
          <td>${fmtDatePt(l.date)}</td>
          <td>${l.subject||'—'}</td>
          <td>${l.topic&&l.topic!==l.subject?l.topic:'—'}</td>
          <td>${l.duration}min</td>
          <td>${l.meetLink?`<a href="${l.meetLink}" target="_blank">🎥 Link</a>`:'—'}</td>
        </tr>`).join('')}
      </tbody>`
...
function renderCal(month, gridId, labelId, onDayClick, lessons, studentFilter) {
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    html+=`<div class="cd${isToday?' cd-today':''}${hasL?' cd-has':''}" onclick="selectCalDay('${gridId}','${iso}',calCb_${gridId})"><span>${d}</span></div>`;
  }
  grid.innerHTML=html;
  // store callback
  window[`calCb_${gridId}`]=onDayClick;
}

function selectCalDay(gridId, iso, cb) {
...
function lessonItemHTML(l, isTeacher) {
  ...
  return `<div class="lesson-item">
    ...
    <div class="li-info">
      ...
      ${l.meetLink?`<a class="meet-link" href="${l.meetLink}" target="_blank">🎥 Entrar no Google Meet</a>`:''}
    </div>
    <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;flex-shrink:0">
      <span class="badge b-sched">Agendada</span>
      ${isTeacher?`<button class="btn-sm" onclick="markLessonDone(${l.$loki})">✅</button><button class="btn-icon danger" onclick="deleteLesson(${l.$loki})">🗑</button>`:''}
    </div>
  </div>`;
}

function dayEventHTML(l, isTeacher) {
  return `<div class="dev ${l.status==='done'?'dev-done':''}">
    <div class="dev-topic">📖 ${l.subject||l.topic}</div>
    <div class="dev-meta">⏰ ${l.time} • ${l.duration}min${isTeacher?` • 👤 ${l.studentName}`:''} • <span class="badge ${l.status==='done'?'b-done':'b-sched'}">${l.status==='done'?'Realizada':'Agendada'}</span></div>
    ${l.meetLink?`<a class="meet-link" href="${l.meetLink}" target="_blank">🎥 Google Meet</a>`:''}
    ${l.topic&&l.topic!==l.subject?`<div style="font-size:12px;color:var(--g500);margin-top:4px">${l.topic}</div>`:''}
    ${isTeacher&&l.status==='scheduled'?`<div style="display:flex;gap:6px;margin-top:8px"><button class="btn-sm" onclick="markLessonDone(${l.$loki})">✅ Marcar realizada</button><button class="btn-icon danger" onclick="deleteLesson(${l.$loki})">🗑</button></div>`:''}
  </div>`;
}
...
// Override openModal to auto-init signature pads
const _origOpenModal = openModal;
window.openModal = function(id) {
  _origOpenModal(id);
  if (id === 'modal-cert')         setTimeout(() => initSigPad('sig-teacher'), 80);
